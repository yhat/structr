...
test_that("keys return a list of the keys", {
  ...
  x['b'] = 2
  x['c'] = 3
  expect_that(x$keys(), equals(list.py('a', 'b', 'c')))
})

test_that("keys returns an empty list when dict is blank", {
  x <- dict()
  expect_that(x$keys(), equals(list.py()))
})
...
test_that("values returns a pylist of the values", {
  ...
  x['b'] = 2
  x['c'] = 3
  expect_that(x$values(), equals(list.py(1, 2, 3)))
})
...
test_that("values the right values", {
  ...
  expect_that(x$values(), equals(list.py(1, 2, 3)))
  x$pop('b')
  expect_that(x$values(), equals(list.py(1, 3)))
})
...
test_that("popitem removes & returns a value if the key exists", {
  ...
  x['b'] = 2
  x['c'] = 3
  expect_that(x$popitem(), equals(list.py("a", 1)))
  x$defaultvalue <- -1
...
test_that("items returns a list of zipped keys/values", {
  x <- dict("a"=1, "b"=2)
  expect_that(x$items(), equals(list.py(list.py("a", 1), list.py("b", 2))))
})
